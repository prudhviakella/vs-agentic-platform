...
THE CORE IDEA -- relevant retrieval, not full state passing:
  ...
  retrieving only the TOP 3 MOST RELEVANT ones for the current question:
    ...
    Returns top 3 semantically similar past Q&As
          |
    Injected as SystemMessage into state
          |
  ...
  HOW episodic memory and summarization work together:
    SummarizationMiddleware fires when state grows beyond 3,000 tokens.
    It compresses old messages -- including old episodic SystemMessages.
    But nothing is lost -- the actual memories live permanently in Pinecone.
    The next turn fetches fresh relevant memories and injects them again.

    State (in memory)   -> always small, summarized, bounded (working memory)
    Pinecone (episodic) -> permanent, full fidelity, searchable (long-term memory)
...
class EpisodicMemoryMiddleware(BaseAgentMiddleware):
    ...
    def __init__(self, store: BaseStore):
        super().__init__()
        # Shared with build_agent(store=...) -- writes here are immediately
        # visible on the next turn since both use the same PineconeStore instance
        self._store = store
    ...
    def before_agent(self, state: AgentState, runtime: Runtime) -> dict[str, Any] | None:
        """
        Search episodic memory and inject relevant past interactions as a
        SystemMessage into state so the LLM sees them directly.

        WHY return as a SystemMessage (not a state key):
          The LLM sees the messages list -- a SystemMessage injected here is
          visible to the LLM just like the base system prompt. No @dynamic_prompt
          wiring needed, no second Pinecone call.

          Message history the LLM sees:
            SystemMessage -> base system prompt
            SystemMessage -> "Relevant past interactions..."  <- injected here
            HumanMessage  -> current question
            AIMessage     -> LLM answer

        WHY query=question (not query=""):
          query=""       -> recency fetch -- returns 3 most RECENT entries.
                            Ignores what the user is asking right now.
          query=question -> semantic search -- returns 3 most RELEVANT past
                            memories based on the current question.
        """
        user_id = self._get_user_id(runtime)

        # Extract the current human question to use as the search query
        ...
        # Format memories as numbered blocks and inject as a single SystemMessage
        context_lines = []
        ...
        context = "\n\n".join(context_lines)
        log.info(f"[EPISODIC] Injecting {len(items)} memories into context  user={user_id}")
    ...
    def _parse_storage_decision(answer: str) -> tuple[bool, str]:
        ...
        We strip it before returning the answer to the user -- it is internal
        metadata they should never see.
